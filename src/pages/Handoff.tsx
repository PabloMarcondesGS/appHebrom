...
export const Handoff: React.FC = () => {
    ...
    const started = async () => {
        ...
        .then(async (response) => {
            if(response.data.Contratos.resultado) {
                setNotFound(response.data.linkRedeCredenciada);
            }
            
            await AsyncStorage.setItem('tokenUser', response.data.token);
            setTitles(response.data.Contratos);

            await AsyncStorage.setItem('Ouvidoria', JSON.stringify(response.data.Contratos.linkRedeCredenciada));
  ...
  const handleNextScream = async (linkRedeCredenciada: any) => {
    await AsyncStorage.setItem('linkRedeCredenciada', linkRedeCredenciada.toString());

    console.log(linkRedeCredenciada.toString());
  }
return (
          ...
          <Image source={require('../assets/logo.png')}  style={styles.logo}/>
              ...
              {notFound? (
                  ...
                  titles.map((data: commomQ) => (
                      <TouchableOpacity style={styles.card} onPress={() =>  Linking.openURL(data.linkRedeCredenciada)} key={data.linkRedeCredenciada}>
                        {/* <Feather name="link" color={colors.gray60} size={24} style={styles.navBarIcon}  /> */}
                        <Text style={styles.cardTitleText} onPress={() => Linking.openURL(data.linkRedeCredenciada)} >Rede Credenciada</Text>
                      </TouchableOpacity>
                  )) 
              }
          </View>
